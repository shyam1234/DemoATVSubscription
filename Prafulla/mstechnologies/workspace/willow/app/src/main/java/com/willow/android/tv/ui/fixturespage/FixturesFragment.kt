...
import com.willow.android.tv.ui.fixturespage.viewmodel.FixturesViewModelFactory
...
class FixturesFragment() : BaseFragment(), KeyListener {
...
// onDestroyView.
    ...
    private var dateFragment: FixturesByDateFragment? = null
    private var seriesFragment: FixturesBySeriesFragment? = null
    private val tabsArray = mutableListOf<String>(
        "By Date",
        "By Series",
    )
    ...
    init {
        Timber.d("called FixturesFragment")
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        navigationMenuCallback?.navMenuScreenName(Types.ScreenName.FIXTURES)
        mViewModel = ViewModelProvider(
            this,
            FixturesViewModelFactory(WillowApplication.instance, data)
        ...
        mViewModel?._targetUrl?.observe(viewLifecycleOwner) {
            GlobalTVConfig.currentPage = resources.getStringArray(R.array.arrayPages)[3]
    ...
    private fun setUpTabsnViewPager(tabsList: MutableList<String>) {
        ...
        tabRadioGroup.setOnCheckedChangeListener { group, checkedId ->
            ...
            when (radioButton.tag) {
                "By Date" ->{
                    dateFragment = FixturesByDateFragment.newInstance(this)
                    navigateToFragment(dateFragment!!)
                    checkedPosition = BY_DATE_POSITION
                }

                "By Series" ->{
                    seriesFragment = FixturesBySeriesFragment.newInstance(this)
                    ...
                    val seriesFragment = FixturesBySeriesFragment.newInstance(this)
                    navigateToFragment(seriesFragment)
                }
            }
        }
    ...
    private fun setupTimerForLiveRefresh() {
        ...
        pollerTimer = object: CountDownTimer(oneDayMillis, pollerInterval*1000) {
            override fun onTick(millisUntilFinished: Long) {
                mViewModel?.loadFixturesPageConfig()
            }
